...
--- Retrieve the time stamp information (Same as pkt:getTimestamp but return struct).
--- @return The timestamp or nil if the packet was not time stamped.
function pkt:getTimestampTS(dev)
	if bit.bor(self.ol_flags, dpdk.PKT_RX_IEEE1588_TMST) ~= 0 then
		local data = ffi.cast("uint32_t* ", self:getData())
		local low, high
		if dev and dev.embeddedTimestampAtEndOfBuffer then
...
pkt.getVxlanUdpPacket = createStack("eth", "ip4", "udp", "vxlan", { "eth", name = "nestedEth" }, {"ip4", name = "nestedIp4"}, {"udp", name = "nestedUdp"})
...
pkt.getGreUdpPacket = createStack("eth", "ip4", "gre", {"ip4", name = "nestedIp4"}, {"udp", name = "nestedUdp"})

pkt.getGtpUdpPacket = createStack("eth", "ip4", "udp", "gtp", {"ip4", name = "nestedIp4"}, {"udp", name = "nestedUdp"})
pkt.getGtpPacket = createStack("eth", "ip4", "udp", "gtp")
