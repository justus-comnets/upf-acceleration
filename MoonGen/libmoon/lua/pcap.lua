...
local function writeHeaderNS(ptr)
	local hdr = headerPointer(ptr)
...
--- Create a new fast pcap writer with the given file name. (Same as mod:newWriter but with ns resolution)
--- Call :close() on the writer when you are done.
--- @param startTime posix timestamp, all timestamps of inserted packets will be relative to this timestamp
---        default: relative to libmoon.getTime() == 0
function mod:newWriterNS(filename, startTime)
	startTime = startTime or wallTime() - libmoon.getTime()
	local fd = S.open(filename, "creat, rdwr, trunc", "0666")
	if not fd then
		log:fatal("could not create pcap file: %s", strError(S.errno()))
	...
	local offset = writeHeaderNS(ptr)
	ptr = cast("uint8_t*", ptr)
	return setmetatable({fd = fd, ptr = ptr, size = size, offset = offset, startTime = startTime}, writer)
end
...
--- Write a packet to the pcap file (same as writer:write, but with ns resolution )
--- @param timestamp relative to the timestamp specified when creating the file
function writer:writeNano(tv_sec, tv_nsec, data, len, origLen)
	if self.offset + len + 16 >= self.size then
		self:resize(self.size * 2)
	end
--	local time = self.startTime + timestamp
--	local timeSecOrg = math.floor(time)
--	local timeMicros = (time - timeSecOrg) * 1000000
--	local timeSec = math.floor(timestamp)
--	local timeNanos = (timestamp - timeSec) * 1000000000
--	log:fatal("timeSec: " .. timeSec)
--	log:fatal("timestamp: " .. timestamp .. " timeNanos: " .. timeSec .. " " .. timeNanos .. "  timeMicros: " .. timeSecOrg .. " " .. timeMicros)
	C.libmoon_write_pcap(self.ptr + self.offset, data, len, origLen or len, tv_sec, tv_nsec)
	self.offset = self.offset + len + 16
end
...
--- Write a mbuf to the pcap file (same as writer:writeBuf, but with ns resolution )
--- @param timestamp relative to the timestamp specified when creating the file
--- @param snapLen truncate the packet to this size
function writer:writeBufNano(tv_sec, tv_nsec, buf, snapLen)
	local size = buf:getSize()
	snapLen = snapLen or size
	self:writeNano(tv_sec, tv_nsec, buf:getData(), min(size, snapLen), size)
end
