...
class PCAPParser:
    ...
    def analyze_core(self, cpcap_file, save_pickle=False, load_pickle=False, core_type="Open5GS", save_pickle_dir=None,
        ...
        assert "Open5GS" in core_type or "Nokia" in core_type or "Fwd" in core_type
        if save_pickle_dir:
        ...
        if load_pickle:
            ...
            if "Fwd" in core_type:
                self.logger.info("Analyze forwarding PCAPs of DuT.")
                parsed_seq_nos = set()
                for ts, buf in cpcap:
                    eth = dpkt.ethernet.Ethernet(buf)
                    ip = eth.data
                    ...
                    sec, nsec, seq_no = struct.unpack('>qqq', udp.data[0:24])
                    if not seq_no in parsed_seq_nos:
                        data_sent_ts.append(ts)
                        data_seq_no_pre.append(seq_no)
                        parsed_seq_nos.add(seq_no)
                    else:
                ...
                for ts, buf in cpcap:
                    ...
                    udp = ip.data
                    if not upload:
                        if udp.sport == GTPU_PORT:  # GTP packet
                            recv_ts_sec, recv_ts_nsec = decimal_to_sec_nsec(ts)
                            data_recv_ts_sec.append(recv_ts_sec)
                            data_recv_ts_nsec.append(recv_ts_nsec)
                        ...
                        if udp.dport == GTPU_PORT:  # GTP packet
                            data_sent_ts.append(ts)
                            sec, nsec, seq_no = struct.unpack('>qqq', udp.data[gtp_offs:gtp_offs + 24])
                            data_seq_no_pre.append(seq_no)
                        else:
...
def show_owd_perc_dist(delta_t_lists, files, save=False):
    fig = plt.figure(tight_layout=True)
    ax = fig.add_subplot(111)
    # ax.set_yscale('log')
    ax.set_xscale('close_to_one', nines=4)
    ax.set_title(r"One-way Delay Percentile Distribution")
    ax.set_ylabel(r'One-way delay (s)')
    ...
    for i, delta_t_list in enumerate(delta_t_lists):
        ...
        ecdf = sm.distributions.empirical_distribution.ECDF(delta_t_list)
        delta_t = np.linspace(min(delta_t_list), max(delta_t_list), num=1000)
        prob = ecdf(delta_t)
        ax.plot(prob, delta_t, label=name)
        ax.legend()

    ax.set_xticklabels([tick * 100 for tick in ax.get_xticks()])
...
def find_obj_files(logdir, core=False):
    ...
    for (dirpath, dirnames, filenames) in os.walk(logdir):
        ...
        break
    if core:
        core_objs = [logdir + file for file in all_objs if ".core." in file and ".obj" in file]
        return core_objs
    else:
        objs = [logdir + file for file in all_objs if ".core." not in file and ".obj" in file]
        return objs
...
def find_pcap_files(logdir, core=False):
    ...
    for (dirpath, dirnames, filenames) in os.walk(logdir):
        ...
        break
    core_pcaps = [logdir + file for file in all_pcaps if "core.pcap" in file]
    pcaps = [logdir + file for file in all_pcaps if "pcap" in file and not ".core." in file]
...
if __name__ == "__main__":
    ...
    if args.show:
        ...
        elif args.type == "owd-perc":
            delta_t_lists = [pp_dict[key].owdelay(*results_dict[key]) for key in results_dict.keys()]
            show_owd_perc_dist(delta_t_lists, args.files, save=args.save)
        else:
